...
> **R:**  El número de líneas es mayor porque el preprocesador expande las directivas #include, incorporando en el archivo resultante el contenido completo de las bibliotecas utilizadas, como stdio.h y stdlib.h. Estas incluyen numerosas declaraciones de funciones, definiciones de macros y tipos de datos adicionales. En consecuencia, el archivo generado contiene no solo el código original, sino también todo el código proveniente de dichas bibliotecas, lo que explica el aumento en la cantidad de líneas.
...
> **R:** El comando no devuelve nada porque el archivo .i es el resultado del preprocesamiento. En esta etapa, el sistema elimina automáticamente todos los comentarios del código antes de pasárselo al compilador. Por lo tanto, el texto "Archivo fuente principal" ya no existe en ese archivo.
...
> **R:** el primero no genera ninguna salida
> printf("[DEBUG] %s\n", ("Iniciando main"));
...
> **R:** Las líneas del tipo # N "archivo" son marcadores que indican en qué archivo y en qué línea original se encuentra el código que aparece a continuación.Se ve que ese bloque proviene del archivo c:\\mingw\\include\\stdio.h. ejemplo: 6:# 1 "c:\\mingw\\include\\stdio.h" 1 3
...
> **R:**LFB14: etiqueta
> .cfi_startproc indica al depurador que comienza una nueva funcion
> pushl	%ebp guarda el valor del registro de la función que llamó a la actual en la pila.
> .cfi_def_cfa_offset 8 indica que el "CFA" (Canonical Frame Address) ahora está a una distancia de 8 bytes desde el puntero de pila actual
...
> **R:**En programa.o, aparece como U (Undefined) porque ahí solo se llama a la función, pero no está implementada en ese archivo. En matematica.o, aparece como T (Text) porque ahí sí está definida la función, es decir, su código está en la sección .text.
...
> **R:** Sí, quedan símbolos U porque pertenecen a librerías externas (como la librería estándar de C o soporte del compilador) que no se “copian” dentro del ejecutable.Estos símbolos los resuelve el cargador dinámico del sistema operativo cuando ejecutás el programa.
...
> **R:**En una macro como CUADRADO(x) el preprocesador reemplaza directamente por su contenido antes de compilar, como si copiara y pegara la expresión en el código. Por ejemplo, donde escribo CUADRADO(3) en realidad queda algo como 3*3 antes de compilar. Por eso desaparece en esa etapa y no tiene control de tipos. En cambio, una función real como sumar(a, b) sí se compila como código, se enlaza y el compilador verifica que los tipos de los parámetros sean correctos.
...
> **R:**Un símbolo de tipo T es código de una función, o sea que está en la sección .text del archivo objeto. En cambio, un símbolo de tipo D es un dato y está en la sección .data.
...
> **R:**   ntdll.dll => /c/Windows/SYSTEM32/ntdll.dll (0x7ffec1590000)
        ntdll.dll => /c/Windows/SysWOW64/ntdll.dll (0x776e0000)
        wow64.dll => /c/Windows/System32/wow64.dll (0x7ffebf6e0000)
        wow64win.dll => /c/Windows/System32/wow64win.dll (0x7ffebf5b0000)

¿Por qué `libc` no hubo que especificarla explícitamente al enlazar con `gcc`?

> **R:**Porque gcc automáticamente agrega la librería estándar de C (libc) al momento de enlazar
